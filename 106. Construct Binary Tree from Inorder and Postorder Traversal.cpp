...
TreeNode *solve(vector<int> postorder,int postStart,int postEnd,vector<int> inorder,int inStart,int inEnd, map<int,int>&mp){
        ...
        root->left = solve(postorder,postStart,postStart+nelem-1,inorder,inStart,elem-1,mp);
        root->right = solve(postorder,postStart+nelem,postEnd-1,inorder,elem+1,inEnd,mp);
        return root;
